...
static void dump_block_bytes(void *data, size_t nbytes) {
  for (size_t i = 0; i < nbytes; i++) {
    uint64_t v = ((unsigned char *)data)[i];
    print_out("%lu\n", &v, sizeof(uint64_t));
  }
}
...
int main() {
  ...
  size_t data_sz = 128 - sizeof(struct header);

  dump_block_bytes(data1, data_sz);
  dump_block_bytes(data2, data_sz);
