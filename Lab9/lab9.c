...
port number). The IP address is 127.0.0.1 for the local host and port 8000
2. Is it UDP or TCP? How do you know?
  It is TCP because the docket is created with SOCK_STREAM which is for TCP
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? It gets the data from the
keyboard/standard input, the read() call uses STDIN_FILENO to read from stdin
4. How does the client program end? How can you tell that in the code?
  The program ends when 1 or less is returned by read(), I can tell because the
loop condition is num_read > 1, so it exits once num_read is equal or less
than 1.
*/
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(cfd, buf, BUF_SIZE)) > 0) {
    ...
    buf[num_read] = '\0';
    printf("Client %d: Message #%d: %s", client_id, current_count, buf);
  }
  close(cfd);
